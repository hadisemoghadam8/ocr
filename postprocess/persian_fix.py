...
def improve_persian_text(text: str) -> str:
    ...
    # --------------------------------------------------
    # ✅ ۱۴. اصلاح خطای رایج OCR: _ → د
    # --------------------------------------------------
    # فقط "د" تک‌کاراکتری بین دو کلمه فارسی را به _ برمی‌گرداند
    # روی کلمات عادی (مثل داد، دبستان، صد) هیچ تاثیری ندارد
    text = re.sub(r'(?<=[\u0600-\u06FF])\s+د\s+(?=[\u0600-\u06FF])', ' _ ', text)
    
    # حذف تکرارهای احتمالی آندرلاین و فاصله‌های اضافه
    ...
    # فقط اگر متن حداقل ۳ کاراکتر فارسی دارد و با حرف تمام شده
